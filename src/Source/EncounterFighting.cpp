	...
	EncounterState(parentEncounter)
{
	...
	// Load sounds
	m_missedAttack.loadFromFile(FilePaths::SOUND_MISSED);
...
void EncounterFighting::ShowSucceededAttack()
{
...
void EncounterFighting::ShowTotalDamage()
{
	...
	//else
	baby.ModifyHappiness(std::min(-baby.GetHappiness() / 3, -5));

	m_parentEncounter->DamageMonster(m_upcomingDamage);
}

void EncounterFighting::ShowMissedAttack()
{
	// Update the text to MISS and
	...
	m_attackSound.setBuffer(m_missedAttack);
	m_attackSound.play();
}
